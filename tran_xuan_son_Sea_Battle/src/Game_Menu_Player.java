...
public class Game_Menu_Player {
    public static List<Save_Ranking> List_Ranking = new ArrayList<>(); // chỉ push ng thắng vô, sắp xếp.
            ...
            for (int i = 0; i <= 1; i++)
            {
                ...
                if (cnt == 0) {
                    ...
                    else
                    {
                        ...
                        // Thêm vô file data ranking.
                        Save_Ranking sr = new Save_Ranking(Main.List.get(i).Name,Main.List.get(i).Turn);
                        List_Ranking.add(sr);
    ...
    public void Write_Ranking(){
        File file = new File("C:\\Users\\Admin\\Desktop\\Java PRO\\BTVN\\BTGiuaKhoa\\tran_xuan_son_Sea_Battle\\List Ranking\\dataranking.txt");
        try {
    ...
    public void Show_Ranking(){
        ...
        // Đọc tất cả ca đối tượng Player đc lưu từ ranking
        ...
        int cnt = 1;
        for(Save_Ranking sv : List_Ranking){
            System.out.println(String.format("| "+ "%-3s" + " | " + "%-13s" + " | " + "%-9s" + " |", cnt, sv.getName(), sv.getTurn()));
            cnt ++;
            System.out.println("-----------------------------------");
        }
    }
